...
const ExpenseForm = (props) => {
  ...
  return (
    <form onSubmit={submitHandler} className='grid items-center justify-center grid-cols-3 p-4 mx-8 my-10 border-4 border-teal-700 shadow-neon-glow-green md:space-y-0 md:space-x-2 rounded-2xl bg-violet-500' action="input">
        ...
        <button type='button' onClick={props.action} className="w-48 h-10 col-span-2 px-2 mr-2 text-xl font-bold text-center text-white bg-teal-700 border-2 md:col-span-1 hover:bg-teal-500 rounded-xl" >Cancel</button>
        </div>
    </form>
    )
}
